...
def plot_shakemap(sources, norths, easts, values_list, filename, folder,
    ...
    if plot_values is True:
        ...
        pgas_waveforms, stations = get_measured_values.get_max_pga(events, folder_waveforms, stations)
        st_lats, st_lons = [], []
        if scale == "mm":
            print(pgas_waveforms)
    ...
    if quantity == "velocity":
        if scale == "mm":
            ...
            m.set_array(values)
            m.set_clim(0., vmax)
            plt.colorbar(m, boundaries=num.linspace(0, vmax, 6),  label='PGV [mm/s]')
        else:
    ...
    img_extent_pyrocko = (num.min(lats_map)+img_puffer, num.max(lats_map)+img_puffer, num.min(lons_map)-img_puffer, num.max(lons_map)-img_puffer)
